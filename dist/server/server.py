...
cnx = mysql.connector.connect(user='root', password='1510',database='lms',charset='utf8')
    ...
    try:
        ...
        if(isBorrower):
            ...
            if(isCheckedOut):
                ...
                dueDate = dateOut + timedelta(days=14)
...
def calculateFines():
    response = None
    query = 'select loan_id,due_date,date_in from BOOK_LOANS where (date_in IS NOT NULL AND date_in > due_date) OR (date_in IS NULL AND CURDATE() > due_date)'
    cursor.execute(query)
    ...
    for row in rows:
        if(row[2] == None):
            diff = today.date() - row[1].date()
            fine = round(diff.days * 5,2)
        else:
            diff = row[2] - row[1]
            fine = round(diff.days * 5,2)
        try:
